	...
	for i, f := range fileList {
		...
		if nonstrict {
			if filepath.Ext(fileName) == ".srt" {
				writeStringToFile(outputFile, fileName+"\t"+"Субтитры"+"\t"+"\n", 0775)
				consolePrint(fmt.Sprintf("%"+strconv.Itoa(len(strconv.Itoa(fileListLength)))+"d", i+1) + "/" + strconv.Itoa(fileListLength) + "  " + truncPad(fileName, 64, 'l') + "  " + "Субтитры" + "\n")
				continue
			} else {
				// Get file duration.
		...
		if filepath.Ext(fileName) == ".srt" {
			writeStringToFile(outputFile, movieName+"\t"+coid+"\t"+"Субтитры"+"\t"+"\t"+fileName+"\n", 0775)
			consolePrint(fmt.Sprintf("%"+strconv.Itoa(len(strconv.Itoa(fileListLength)))+"d", i+1) + "/" + strconv.Itoa(fileListLength) + "  " + truncPad(movieName, 32, 'l') + "  " + truncPad(coid, 8, 'l') + "  " + "Субтитры" + "  " + truncPad(fileName, 32, 'l') + "\n")
		} else {
			// Get file duration.
			file, err := ffinfo.Probe(f)
			if err != nil {
